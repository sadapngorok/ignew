...
def ___menu___():
    try:
        ...
        exit(f"{P}[{K}!{P}]{K} Connection Error")
    print(f"{K}[{M}1{K}]{B} Dump Username From Following")
...
def ___proxy___():
    try:
        ___roz = requests.get('https://github.com/MN4WN1-777/ignew/blob/master/Data/proxy.txt').text
...
class ___crack___:
    ...
    def __init__(self):
        ...
        self.ok = []
        self.cp = []
        print(f"\n{K}[{M}1{K}]{P} Enter Password {B}[{H}Name, Name123, Name12345{B}]{K}")
        print(f"{K}[{M}2{K}]{P} Enter Password {B}[{H}name, name123, name1234, name12345{B}]{K}")
        print(f"{K}[{M}3{K}]{P} Enter Password {B}[{M}name, name123, name1234, name12345, name123456{B}]{K}")
        print(f"{K}[{M}4{K}]{H} Enter Password Manual {H}[{M}>5{H}]{K}\n")
